...
def argmax_gap(nums: list[int]) -> tuple[HUE, HUE]:
    assert len(nums) >= 2

    nums = sorted(nums, reverse=True)
    max_gap = -1
    i1_idx_max, i2_idx_max = 0, 1

    for (i1_idx, i1), (i2_idx, i2) in sliding_window(2, enumerate(nums)):
        if (gap := i1 - i2) <= max_gap:
            continue

        max_gap = gap
        i1_idx_max, i2_idx_max = i1_idx, i2_idx

    return nums[i1_idx_max], nums[i2_idx_max]
...
class LabelwiseColorFilterAugmentation:
    ...
    def assign_to_color(self, label: str) -> COLOR:
        ...
        if num_colors := len(self.labels_to_color) == 0:
            ...
            hue1, hue2 = argmax_gap(exiting_hues)
            hue = (hue1 + hue2) / 2
            hue %= 360
        ...
        self.labels_to_color[label] = colorsys.hsv_to_rgb(hue, s=1., v=1.)
        return self.labels_to_color[label]

    def _augment_frame_with_color(self, frame_CHW: torch.Tensor, color) -> torch.Tensor:
        color_tensor = torch.tensor(color, device=frame_CHW.device).view(3, 1, 1)
    ...
    def augment_video(self, video_TCHW: torch.Tensor, label: str) -> torch.Tensor:
        ...
        label = str(label)
        for frame_idx in range(video_TCHW.shape[1]):
...
def visualize_color_wheel(augmentation, labels):
    ...
    for label in labels:
        ...
        hue = augmentation.labels_to_hues[label] * 360
...
if __name__ == "__main__":
    ...
    for idx, label in enumerate(labels):
        ...
        filtered_frame = filtered_video[:, 0].permute(1, 2, 0)
